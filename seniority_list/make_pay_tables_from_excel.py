...
def create_pay_data(case=None):

    if case is None:
        try:
            case = pd.read_pickle('dill/case_dill.pkl').case.value
            print('using "' + case + '" as case input for create_pay_data')
        except:
            print('pay_data_creation fail')
            return
    ...
    excel_path = 'excel/' + case + '/pay_tables.xlsx'

    xl = pd.read_excel('excel/' + case + '/settings.xlsx')

    scalars = defaultdict(int)

    # ## scalars

    scalars.update(make_dict_from_columns(xl, 'option', 'value'))

    enhanced_jobs = scalars['enhanced_jobs']
    ...
    year = scalars['pay_table_year_sort']
    longevity = scalars['pay_table_longevity_sort']
    ...
    pay_rates = pd.read_excel(excel_path, sheetname='rates')
    # read monthly pay hours per job level and job description from excel file
    pay_hours = pd.read_excel(excel_path, sheetname='hours')
    ...
    if enhanced_jobs:
        ...
        full_suf = scalars['enhanced_jobs_full_suffix']
        part_suf = scalars['enhanced_jobs_part_suffix']
